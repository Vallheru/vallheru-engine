...
{if $View == "market"}
    {$Viewinfo} <a href="{$SCRIPT_NAME}">{$Aback}</a>.<br /><br />
    <form method="post" action="{$SCRIPT_NAME}?view=market&amp;lista=name">
        {$Tname}: <input type="text" name="szukany" />
        <input type="submit" value="{$Asearch}" />
    </form><br />
    <table>
    <tr>
    <td width="100"><a href="{$SCRIPT_NAME}?view=market&amp;lista=name"><b><u>{$Tname}</u></b></a></td>
    <td width="50"><a href="{$SCRIPT_NAME}?view=market&amp;lista=minlev"><b><u>{$Tlevel}</u></b></a></td>
    <td width="50"><a href="{$SCRIPT_NAME}?view=market&amp;lista=amount"><b><u>{$Tamount}</u></b></a></td>
    <td width="100"><a href="{$SCRIPT_NAME}?view=market&amp;lista=cost"><b><u>{$Tcost}</u></b></a></td>
    <td width="50"><a href="{$SCRIPT_NAME}?view=market&amp;lista=owner"><b><u>{$Tseller}</u></b></a></td>
    ...
    {section name=item loop=$Name}
        ...
        <td>
	{if $Owner[item] == $Pid}
	    <a href="market.php?view=myoferts&amp;type=lmarket&amp;delete={$Iid[item]}">{$Adelete}</a><br />
            <a href="market.php?view=myoferts&amp;type=lmarket&amp;change={$Iid[item]}">{$Achange}</a><br />
            <a href="market.php?view=myoferts&amp;type=lmarket&amp;add={$Iid[item]}">{$Aadd}</a>
	{else}
	    <a href={$SCRIPT_NAME}?buy={$Iid[item]}>{$Abuy}</a>
	{/if}
	</td>
	</tr>
    {/section}
    ...
    {if $Tpages > 1}
	    ...
	    {if $page == $Tpage}
	        {$page}
	    {else}
                <a href="{$SCRIPT_NAME}?view=market&page={$page}&amp;lista={$Mlist}">{$page}</a>
	    {/if}
    	{/for}
    {/if}
{/if}
...
{if $View == "add"}
    ...
    <form method="post" action="{$SCRIPT_NAME}?view=add&amp;step=add"><table>
    <tr><td colspan="2">
...
{if $Buy != ""}
    ...
    <form method="post" action="{$SCRIPT_NAME}?buy={$Itemid}&amp;step=buy"><table>
